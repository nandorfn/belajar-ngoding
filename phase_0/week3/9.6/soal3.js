...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    let product = listBarang[i];
    let productName = product[0];
    let productPrice = product[1];
    let productStock = product[2];
    let shopperList = [];
    let totalProfit = 0;

    for  (let j = 0; j < shoppers.length; j++) {
      let customer = shoppers[j];
      
      if (customer.product === productName) {
        if (customer.amount <= productStock) {
          shopperList.push(customer.name);
          totalProfit += productPrice * customer.amount;
          productStock -= customer.amount;
        }
      }
    }
